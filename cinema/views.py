...
class MovieViewSet(
    ...
    @extend_schema(summary="Upload an image to a specific movie")
    ...
    list=extend_schema(
        parameters=[
            OpenApiParameter(
                ...
                description=(
                    "Filter by movie session date (e.g. ?date=2024-08-25)"
                ),
            ),
            OpenApiParameter(
                "movie",
...
@extend_schema_view(
    list=extend_schema(
        description="List all orders made by the current user"
    ),
