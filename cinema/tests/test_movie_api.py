...
class UnauthenticatedMovieApiTests(TestCase):
    ...
    def test_list_movies_denied(self):
        res = self.client.get(MOVIE_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_retrieve_movie_detail_denied(self):
        movie = sample_movie()
        url = detail_url(movie.id)
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_create_movie_denied(self):
        payload = {
            "title": "New Movie",
    ...
    def test_upload_image_denied(self):
        movie = sample_movie()
...
class MovieImageUploadTests(TestCase):
    ...
    def test_post_image_to_movie_list_is_disallowed(self):
        url = MOVIE_URL
        with tempfile.NamedTemporaryFile(suffix=".jpg") as ntf:
            img = Image.new("RGB", (10, 10))
            img.save(ntf, format="JPEG")
    ...
    def test_upload_image_permission_denied_for_regular_user(self):
